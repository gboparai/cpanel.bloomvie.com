...
export class StudentMediaRequestComponent {
  ...
  openModal(data: any) {
    ...
    this.selectedUser = {
      ...
      formattedStartDate: data.startDate ? this.formatDate(data.startDate) : '',
      formattedEndDate: data.endDate ? this.formatDate(data.endDate) : '',
      duration: data.startDate && data.endDate ? this.calculateDays(data.startDate, data.endDate) : 0
    };
  ...
  onPlanSelect(event: any) {
    ...
    this.applicationService
      .getSubscriptionPlansByID(this.onSelectPlan)
      .subscribe((res: any) => {
        setTimeout(() => {
          this.spinner.hide();
        }, 0);
